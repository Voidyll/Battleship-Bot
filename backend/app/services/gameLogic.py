...
def placeShip(data: dict):
    ...
    if not valid.success:
        return (
            {
                "error": "Did not include all headers...",
                "missing-headers": list(valid.missingHeaders),
            },
            400,
    ...
    return (
        {"snapshot": game.to_snapshot(), "player-state": state},
        200,
        {"Content-Type": "application/json"},
    )
...
def fire(data: dict, agent: ai.Agent):
    ...
    autoResolveAiTurn = validate.validateBool(
        "autoResolveAiTurn", data["autoResolveAiTurn"]
    )

    game = g.Game().from_snapshot(snapshot)

    status = game.fire_with_auto_ai_turn(
        player, row, col, ai_player, agent.choose_shot, autoResolveAiTurn
    )

    state = game.get_state(player)

    return (
        {"snapshot": game.to_snapshot(), "status": status, "player-state": state},
        200,
        {"Content-Type": "application/json"},
    )
